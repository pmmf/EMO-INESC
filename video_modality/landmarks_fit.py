...
# from myutils import my_ccc_loss, siamese_kpts_generator, myeval
...
__RES_FN__ = '/data/pmmf/OMG/KPTS/grid_wo_norm_mlayers/'
...
print(x_train.shape, x_val.shape, kpts_train_siamese['frame_0'].shape, combo_val['frame_0'].shape, arousal_train.shape, valence_train.shape, arousal_val.shape, valence_val.shape)
...
for idx, combo in enumerate(combinations):
    ...
    siamese_mlp = siamese_encoder(input_shape=(combo_val['frame_0'].shape[1],), n_dense=dict_combo['n_dense'], hidden_dim=dict_combo['hidden_dim'], l2_reg=dict_combo['l2_reg'])
    top_weightspath=__RES_FN__+ str(idx) + '_weights_top.h5'
    siamese_mlp = myfit(model=siamese_mlp, 
                    x_train=combo_train,
                    y_train=y_train,
                    x_val=combo_val,
                    y_val=y_val,
    ...
    # save all model
    siamese_mlp.load_weights(top_weightspath) 
    top_modelpath=__RES_FN__+ str(idx) + '_model_top_ktps.h5'
    siamese_mlp.save(top_modelpath)
    # siamese_encoder = load_model(top_modelpath, custom_objects={'my_ccc_loss': my_ccc_loss})

    # predictions
    preds = siamese_mlp.predict(combo_val)
    ...
    print("siamese_mlp: ", arousalCCC,arousalmse, valenceCCC, valencemse)

    # save log
    with open(f_grid_fn, "a") as f_grid:
        f_grid.write('\n' + "siamese_mlp: combo: " + str(idx) + ': ' + str(arousalCCC) + ', ' + str(arousalmse) + ', ' + str(valenceCCC) + ', ' + str(valencemse))
